...
class CocoDataset(Dataset):
    ...
    def __init__(self, root_dir, set_name="train2017", transform=None, sample_kp=False):
        """
        Args:
        ...
        self.set_heading = "instances_"
        if sample_kp:
            self.set_heading = "person_keypoints_"
        self.coco = COCO(
            os.path.join(
                self.root_dir, "annotations", self.set_heading + self.set_name + ".json"
            )
        )
...
class CSVDataset(Dataset):
    ...
    def _read_annotations(self, csv_reader, classes):
        ...
        for line, row in enumerate(csv_reader):
            ...
            x1 = self._parse(
                math.ceil(float(x1)), int, "line {}: malformed x1: {{}}".format(line)
            )
            y1 = self._parse(
                math.ceil(float(y1)), int, "line {}: malformed y1: {{}}".format(line)
            )
            x2 = self._parse(
                math.ceil(float(x2)), int, "line {}: malformed x2: {{}}".format(line)
            )
            y2 = self._parse(
                math.ceil(float(y2)), int, "line {}: malformed y2: {{}}".format(line)
            )
