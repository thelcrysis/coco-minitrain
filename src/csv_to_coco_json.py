...
def main(args=None):
    parser = argparse.ArgumentParser(
        description="Pascal CSV to COCO JSON format converter."
    )

    parser.add_argument(
        "--csv_train",
        help="Sampled minitrain csv file.",
        default="mscoco_sampled_0.1131.csv",
    )
    parser.add_argument(
        "--csv_classes",
        help="COCO class labels csv file.",
        default="coco_class_labels.csv",
    )
    parser.add_argument(
        "--coco_path",
    ...
    parser.add_argument(
        "--save_json_file_name", help="Save file name", default="mini_coco.json"
    )
